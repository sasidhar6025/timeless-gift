...
export default function SoundPlayer({ src = "/sound/ambient.mp3", title = "Ambient soundtrack" }) {
  ...
  const [volume, setVolume] = useState(() => {
    try {
      return Number(localStorage.getItem(`${STORAGE_KEY}.vol`)) || 0.25;
    } catch {
      return 0.25;
    }
  });
  ...
  // Initialize audio + audio context
  useEffect(() => {
  ...
  // 🔑 EXPOSE DIRECT PLAY / PAUSE (USER-GESTURE SAFE)
  useEffect(() => {
    window.__timelessAudio = {
      play: async () => {
        try {
          const audio = audioRef.current;
          if (!audio) return;
          ...
          setAutoplayBlocked(true);
          setLastPlayError(err?.message || String(err));
        }
      },
    ...
    return () => {
      delete window.__timelessAudio;
    };
  }, []);

  return (
    <>
      {/* Minimal visible control (optional) */}
      <div style={{ position: "fixed", right: 12, bottom: 12, zIndex: 9999 }}>
        <button
          onClick={() => {
            if (playing) window.__timelessAudio?.pause();
            else window.__timelessAudio?.play();
          }}
        >
          {playing ? "Pause" : "Play"}
        </button>
      ...
      {autoplayBlocked && (
        <div style={{ position: "fixed", right: 12, bottom: 60, zIndex: 9999, background: "#000", color: "#fff", padding: 10, borderRadius: 8 }}>
          <div style={{ marginBottom: 6 }}>Enable background sound</div>
          {lastPlayError && <div style={{ fontSize: 12, opacity: 0.7 }}>{lastPlayError}</div>}
          <button onClick={() => window.__timelessAudio?.play()}>Enable</button>
        </div>
      )}
    </>
  );
